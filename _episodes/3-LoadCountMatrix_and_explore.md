...
- The gene-count matrix was generated in the folder ```~/base_path/working_directory/results/star_salmon/salmon.merged.gene_counts.tsv``` using the nfcore-rnaseq pipeline. The input to this pipeline were sub-setted fastq files from 6 samples. 
...
- This file was downloaded previously along with the other files from cloudstor and is named as ```Full_count_matrix.txt```. It is available in the path ```~/base_path/working_directory/Full_count_matrix.txt```
